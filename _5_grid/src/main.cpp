...
void key_callback(GLFWwindow* w, int key, int sc, int action, int mods) {
    ...
    if (key == GLFW_KEY_B) { g_battery = std::min(2.0f, g_battery + 0.05f); std::cout << "battery=" << g_battery << std::endl; }
    if (key == GLFW_KEY_N) { g_battery = std::max(0.0f, g_battery - 0.05f); std::cout << "battery=" << g_battery << std::endl; }
}
...
int main() {
    ...
    GLFWwindow* window = glfwCreateWindow(800, 600, "Grid Only", nullptr, nullptr);
    ...
    GLuint program = createShaderProgram("shaders/fullscreen.vert", "shaders/grid_only.frag");
    ...
    GLint battLoc = glGetUniformLocation(program, "battery");
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (battLoc != -1) glUniform1f(battLoc, g_battery);
